...
public:
  ...
  void addTo(const int& i, const int& m){
    Node* n = nth_node(i);
    detach(n);
    n->value += m;
    Node* current = head->prev;
    while(current != head && current->value > n->value) current = current->prev;
    current = current->next;
    attach(n, current);
  }


  void interseccion(const ListLinkedDouble& aux){
    Node* current = head->next;
    Node* current_aux = aux.head->next;
    while(current != head){
      Node* next = current->next;
      Node* next_aux = current_aux->next;
      if(current->value < current_aux->value){
        detach(current);
        delete current;
        current = next;
      } else if(current->value > current_aux->value){
  ...
  void estremecer(){
    ...
    Node* last = head;
    int i = 0;
    while(current != head && i < num_elems){ 
      Node* next = current->next;
